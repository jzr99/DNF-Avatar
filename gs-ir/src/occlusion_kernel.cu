...
__global__ void SH_reconstruction_kernel(const uint32_t num_rays, const uint32_t sh_degree,
    ...
    for (uint32_t i = 0u; i < num_samples; ++i) {
        ...
        for (uint32_t c = 0u; c < channels; ++c) {
            ...
            accum += 2.5033429417967046f * xy * (xx - yy) * coeffs[16];
            accum += -1.7701307697799304f * yz * (3.0f * xx - yy) * coeffs[17];
            accum += 0.9461746957575601f * xy * (7.0f * zz - 1.0f) * coeffs[18];
            accum += -0.6690465435572892f * yz * (7.0f * zz - 3.0f) * coeffs[19];
            accum += 0.10578554691520431f * (zz * (35.0f * zz - 30.0f) + 3.0f) * coeffs[20];
            accum += -0.6690465435572892f * xz * (7.0f * zz - 3.0f) * coeffs[21];
            accum += 0.47308734787878004f * (xx - yy) * (7.0f * zz - 1.0f) * coeffs[22];
            accum += -1.7701307697799304f * xz * (xx - 3.0f * yy) * coeffs[23];
            accum += 0.6258357354491761f * (xx * (xx - 3.0f * yy) - yy * (3.0f * xx - yy)) * coeffs[24];
