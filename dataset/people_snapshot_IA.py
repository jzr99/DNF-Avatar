...
class PeopleSnapshotDataset(Dataset):
    def __init__(self, cfg, split='train'):
        ...
        camera = np.load(os.path.join(self.root_dir, self.subject, "cameras.npz"))
        ...
        if split == 'predict':
            ...
            if split == "val" and len(self.smpl_params["body_pose"]) != len(img_files):
                self.smpl_params["body_pose"] = self.smpl_params["body_pose"][:len(img_files)]
                self.smpl_params["global_orient"] = self.smpl_params["global_orient"][:len(img_files)]
                self.smpl_params["transl"] = self.smpl_params["transl"][:len(img_files)]
                print("[val] Inconsistent number of SMPL parameters. hacked")
            if len(self.smpl_params['body_pose']) != len(img_files):
                cached_path = os.path.join(root, f"poses/anim_nerf_train.npz")
                print(f" change Loading from", cached_path)
                self.smpl_params = load_smpl_param(cached_path)
