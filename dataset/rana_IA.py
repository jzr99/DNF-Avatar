...
class RANADataset(Dataset):
    def __init__(self, cfg, split='train'):
        ...
        if split == 'train':
            ...
            self.dir_split = cfg.train_split
        elif split == 'val':
            frames = self.val_frames
            self.dir_split = cfg.val_split
        elif split == 'test':
            frames = self.cfg.test_frames[self.cfg.test_mode]
            self.dir_split = cfg.test_split
        elif split == 'predict':
            raise NotImplementedError
            # frames = self.cfg.predict_frames
        else:
            raise ValueError
        root = os.path.join(self.root_dir, self.dir_split, self.subject)
        with open(os.path.join(root, "cameras.json"), "r") as f:
            camera = json.load(f)

        if self.dir_split == "test":
        ...
        albedo_dir = os.path.join(subject_dir, 'albedos')
        normal_dir = os.path.join(subject_dir, 'normals')
    ...
    def readPointCloud(self,):
        if self.cfg.init_mode == 'random':
            ply_path = os.path.join(self.root_dir, self.dir_split, self.subject, 'random_pc.ply')
            ...
            ply_path = os.path.join(self.root_dir, self.dir_split, self.subject, 'cano_smpl.ply')
            # try:
            #     pcd = fetchPly(ply_path)
            # except:
            verts = self.metadata['smpl_verts']
            ...
            ply_path = os.path.join(self.root_dir, self.dir_split, self.subject, filename + '_cano_IA.ply')
            try:
                pcd = fetchPly(ply_path)
            except:
