...
class PeopleSnapshotDataset(torch.utils.data.Dataset):
    def __init__(self, root, subject, split, config, mode, hdri_filepath=None):
        ...
        if cached_path and os.path.exists(cached_path):
            ...
            if split == "val" and len(self.smpl_params["body_pose"]) != len(self.img_lists):
                self.smpl_params["body_pose"] = self.smpl_params["body_pose"][:len(self.img_lists)]
                self.smpl_params["global_orient"] = self.smpl_params["global_orient"][:len(self.img_lists)]
                self.smpl_params["transl"] = self.smpl_params["transl"][:len(self.img_lists)]
                print("[val] Inconsistent number of SMPL parameters. hacked")
            if len(self.smpl_params["body_pose"]) != len(self.img_lists):
                print(f"[{split}] Inconsistent number of SMPL parameters.")
                cached_path = os.path.join(root, f"poses/anim_nerf_train.npz")
                print(f"[{split}] Loading from", cached_path)
                self.smpl_params = load_smpl_param(cached_path)
                assert len(self.smpl_params["body_pose"]) == len(self.img_lists)
