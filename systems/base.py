...
class BaseSystem(pl.LightningModule, SaverMixin):
    ...
    def on_train_batch_start(self, batch, batch_idx, unused=0):
        ...
        if self.C(self.config.system.loss.lambda_in_surface) > 0:
            self.model.with_in_surface_loss = True
        else:
            self.model.with_in_surface_loss = False
    ...
    def on_test_batch_start(self, batch, batch_idx, dataloader_idx):
        ...
        # if batch_idx==0:
        #     print("Visualizing deformed mesh with da pose")
        #     da_smpl_param={}
        #     da_smpl_param['index'] = batch['index']
        #     da_smpl_param['betas'] = batch['betas']
        #     da_smpl_param['body_pose'] = torch.zeros_like(batch['body_pose'], device=batch['body_pose'].device)
        #     da_smpl_param['global_orient'] = torch.zeros_like(batch['global_orient'], device=batch['global_orient'].device)
        #     da_smpl_param['transl'] = torch.zeros_like(batch['transl'], device=batch['transl'].device)
        #     da_smpl_param['body_pose'][..., 2] = torch.pi / 6
        #     da_smpl_param['body_pose'][..., 5] = -torch.pi / 6
        #     da_mesh=self.model.isosurface_deformed(da_smpl_param)
        #     self.save_mesh(
        #         f"dapose-{self.config.model.geometry.isosurface.method}{self.config.model.geometry.isosurface.resolution}.obj",
        #         **da_mesh,
        #     )
