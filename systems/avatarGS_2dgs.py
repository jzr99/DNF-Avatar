...
from utils.loss_utils import full_aiap_loss_2dgs
...
class AvatarGS:
    def __init__(self, config):
        ...
        # self.GS_config.pipeline.compute_cov3D_python = True
        ...
        if load_ckpt is not None:
            self.scene.load_checkpoint(load_ckpt)
            ckpt_iter = int(load_ckpt.split('/')[-1].split('.')[0][4:])
            self.iter = ckpt_iter + 1
            self.progress_bar = tqdm(range(self.iter, self.opt.iterations), desc="Training progress")
        ...
        # assert self.rendering_type == 'forward_pbr'

        if self.dataset.name == 'people_snapshot':
            # hack here
            self.enable_albedo_adapt = False
        else:
            self.enable_albedo_adapt = True
    ...
    def validation_pbr(self, iteration, testing_iterations, testing_interval, scene: Scene, evaluator, renderArgs):
        ...
        validation_configs = [{'name': 'train', 'cameras': [idx for idx in range(0, len(scene.train_dataset),
                                                                                 len(scene.train_dataset) // 10)]},]
        # if scene.distill_dataset is not None:
        #     validation_configs.append({'name': 'distill', 'cameras': [idx for idx in range(0, len(scene.distill_dataset))]})
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, data_idx in enumerate(config['cameras']):
                    ...
                    # self.GS_config.pipeline.compute_cov3D_python = True
                    # assert self.GS_config.pipeline.compute_cov3D_python == True
                    render_output = render_2dgs(data, pc, self.GS_config.pipeline,
                                                self.background, scaling_modifier=1.0,
                    ...
                    gt_mask = torch.clamp(data.original_mask.to("cuda"), 0.0, 1.0)
                    opacity_image = torch.clamp(render_output["rend_alpha"], 0.0, 1.0)

                    wandb_img = wandb.Image(gt_mask[None],
                                            caption=config['name'] + "_view_{}/gt_mask".format(data.image_name))
                    ...
                    w2c = data.w2c_opencv
                    normal_map = torch.clamp(self.transform_normals(w2c, render_output["rend_normal"]) * 0.5 + 0.5, 0.0, 1.0)
                    ...
                    if self.iter > self.pbr_iteration and self.occlusion_flag and (self.GS_config.enable_occ_type == 'pixel' or self.GS_config.enable_occ_type == 'gaussian'):
                        for i in range(9):
                    ...
                    if self.iter > self.pbr_iteration and self.GS_config.enable_occ_type == 'gaussian':
                        # print("begin to use gaussian splatting to render occlusion")
                        # deformed normal
                        # normal_points = pc.get_normal
                        normal_points = self.flip_normals(pc, c2w)
                        points = pc.get_xyz
                        ...
                        for i in range(9):
                            ...
                            if self.GS_config.enable_occ_1spp == False:
                        ...
                        normal_points = self.scene.gaussians.get_canonical_normal
                        ...
                        # elif self.rendering_type == 'deferred_pbr':
                        #     occlusion = occlusion_per_gs
                        #     irradiance = torch.zeros_like(occlusion)
                    ...
                    if self.rendering_type == 'forward_pbr':
                        ...
                        rough_metal_occ = torch.cat(
                            [pc.get_roughness.reshape(-1, 1),
                             pc.get_metallic.reshape(-1, 1), occlusion.reshape(-1, 1)],
                            dim=-1)
                        rough_metal_occ_output = render_fast_2dgs(data, pc,
                                                                  self.GS_config.pipeline,
                                                                  black_background, scaling_modifier=1.0,
                                                                  override_color=rough_metal_occ)
                        albedo_align_output = render_fast_2dgs(data, pc,
                                                               self.GS_config.pipeline,
                                                               self.background, scaling_modifier=1.0,
                                                               override_color=pc.get_albedo)
                        input_albedo = albedo_align_output["render"]

                        unsigned_normal = self.flip_normals(pc, c2w)

                        normal_align_output_dict = render_2dgs(data, pc,
                                                               self.GS_config.pipeline,
                                                               black_background, scaling_modifier=1.0,
                        ...
                        normal_align_output = normal_align_output / (
                                    normal_align_output.norm(dim=0, keepdim=True) + 1e-10)
                        ...
                        pbr_result = pbr_shading(
                            ...
                            roughness=rough_metal_occ_output['render'][[0]].permute(1, 2, 0),  # [H, W, 1]
                            metallic=rough_metal_occ_output['render'][[1]].permute(1, 2, 0),  # [H, W, 1]
                            tone=False,
                            gamma=True,
                            occlusion=rough_metal_occ_output['render'][[2]].reshape(H, W, 1),
                            irradiance=torch.zeros_like(rough_metal_occ_output['render'][[2]]).reshape(H, W, 1),
                            brdf_lut=self.brdf_lut,
                        ...
                        raise NotImplementedError(f"Unknown rendering type {self.rendering_type}")
    ...
    def forward_GS_pbr_step(self, data_idx):
        ...
        pc, loss_reg, colors_precomp = self.scene.convert_gaussians(data, self.iter, compute_loss=True)
        self.precompute_loss_reg = loss_reg
        self.precompute_deformed_gaussian = pc
        self.precompute_colors_precomp = colors_precomp
        render_pkg = {}
        render_pkg['deformed_gaussian'] = pc
        return render_pkg, data

    def forward_GS_pbr_batch_step(self, batch):
        # data = self.scene.train_dataset[data_idx]
        data = batch
        self.iter += 1
        self.iter_start.record()
    ...
    def optimize_GS_pbr_step(self, render_pkg_old, data, distill_data):
        ...
        # self.GS_config.pipeline.compute_cov3D_python = True
        # assert self.GS_config.pipeline.compute_cov3D_python == True
        render_output = render_2dgs(data, self.precompute_deformed_gaussian, self.GS_config.pipeline, self.background, scaling_modifier = 1.0, override_color = self.precompute_colors_precomp)
        ...
        lambda_mask = self.GS_C(self.iter, self.GS_config.opt.lambda_mask)
        use_mask = lambda_mask > 0.
        # TODO check whether the gradient can be back propagated
        opacity = render_output["rend_alpha"] if use_mask else None
        ...
        if lambda_aiap_xyz > 0. or lambda_aiap_cov > 0.:
            loss_aiap_xyz, loss_aiap_cov = full_aiap_loss_2dgs(self.scene.gaussians, self.precompute_deformed_gaussian)
        else:
        ...
        r = 3.0
        scale_reg = self.precompute_deformed_gaussian.get_scaling
        # scale_reg = self.scene.gaussians.get_scaling
        max_scale, _ = torch.max(scale_reg, dim=1)
        min_scale, _ = torch.min(scale_reg, dim=1)
        scale_ratio = torch.maximum(max_scale / min_scale, torch.ones_like(max_scale) * r)
        anisotropy_reg = torch.mean(scale_ratio - r)
        loss += anisotropy_reg

        lambda_scale_reg = self.GS_C(self.iter, self.GS_config.opt.get('lambda_scale_reg', 0.))
        # scale regularization loss
        scale_reg = self.precompute_deformed_gaussian.get_scaling
        scale_penalty = torch.maximum(scale_reg - 0.020, torch.zeros_like(scale_reg))
        scale_penalty = scale_penalty.mean()
        loss += scale_penalty * lambda_scale_reg
        ...
        if distill_data is not None and lambda_normal_similarity > 0.:
            if 'valid_cano' in distill_data.keys():
                valid = distill_data['valid_cano'].detach()
                normal_world_IA = distill_data['normal_cano'].detach()
                normal_points_GS = self.scene.gaussians.get_canonical_normal
                ...
                normal_points_GS = self.precompute_deformed_gaussian.get_normal
                loss_normal_similarity = -F.cosine_similarity(normal_world_IA[valid], normal_points_GS[valid],
                                                              dim=-1).mean()
                # TODO check the shape of the similarity loss
                loss += lambda_normal_similarity * loss_normal_similarity
        ...
        if distill_data is not None and lambda_pos_distill > 0.:
            if 'valid_cano' in distill_data.keys():
                sdf = distill_data['sdf_cano'].reshape(-1, 1).detach()
                valid = distill_data['valid_cano'].detach()
                normal_world_IA = distill_data['normal_cano'].detach()
                target_positions = (normal_world_IA * -sdf) + self.scene.gaussians.get_xyz
                # compute MSE loss
                loss_pos_distill = F.mse_loss(self.scene.gaussians.get_xyz[valid], target_positions[valid])
                ...
                valid = distill_data['valid'].detach()
                normal_world_IA = distill_data['normal_world'].detach()
                target_positions = (normal_world_IA * -sdf) + self.precompute_deformed_gaussian.get_xyz
                # compute MSE loss
                loss_pos_distill = F.mse_loss(self.precompute_deformed_gaussian.get_xyz[valid], target_positions[valid])
                # TODO check the shape of the mse loss
                loss += lambda_pos_distill * loss_pos_distill
        ...
        if distill_data is not None and lambda_materials > 0.:
            if 'valid_cano' in distill_data.keys():
                valid = distill_data['valid_cano'].detach()
                materials_IA = distill_data['materials_cano'].detach()
            else:
                valid = distill_data['valid'].detach()
                materials_IA = distill_data['materials'].detach()
            albedo_GS = self.precompute_deformed_gaussian.get_albedo
            roughness_GS = self.precompute_deformed_gaussian.get_roughness
            metallic_GS = self.precompute_deformed_gaussian.get_metallic
        ...
        # distill image-based normal & depth & albedo
        black_color = [0, 0, 0]
        black_background = torch.tensor(black_color, dtype=torch.float32, device="cuda")
        unsigned_normal = self.flip_normals(self.precompute_deformed_gaussian, c2w)

        normal_align_output_dict = render_2dgs(data, self.precompute_deformed_gaussian, self.GS_config.pipeline,
                                               black_background, scaling_modifier=1.0,
                                               override_color=unsigned_normal)
        ...
        if data.normal_img is not None and lambda_distill_image > 0.:
            ...
            # TODO check whether they are in the same space
            gl_normal_img = self.transform_normals(w2c, normal_align_output_dict["render"]) * 0.5 + 0.5
            # gl_normal_img = self.transform_normals(w2c, render_output["rend_normal"]) * 0.5 + 0.5
            # import pdb; pdb.set_trace()
            ...
            # # todo here the render_pkg["depth_map"] is unknown, we need to check whether data.depth_value is also unknown
            # loss_image_based += l1_loss(render_output["surf_depth"].reshape(-1)[valid_image_mask], data.depth_value.reshape(-1)[valid_image_mask])
            loss += loss_image_based * lambda_distill_image


        # normal orientation loss
        loss_orientation = torch.tensor(0.).cuda()
        lambda_orientation = self.GS_C(self.iter, self.GS_config.opt.get('lambda_orientation', 0.1))
        if lambda_orientation > 0:
            black_color = [0, 0, 0]
            black_background = torch.tensor(black_color, dtype=torch.float32, device="cuda")
            noflip_normal = self.precompute_deformed_gaussian.get_normal
            noflip_normal_output = render_2dgs(data, self.precompute_deformed_gaussian, self.GS_config.pipeline,
                                                   black_background, scaling_modifier=1.0,
                                                   override_color=noflip_normal)
            normal_map_noflip = noflip_normal_output["render"].permute(1, 2, 0) # [H, W, 3]
            loss_orientation = (-view_dirs * normal_map_noflip).sum(dim=-1).clamp(min=0.0).mean()
            loss += lambda_orientation * loss_orientation
        ...
        loss_albedo_image_based = torch.tensor(0.).cuda()
        loss_l1_pbr = torch.tensor(0.).cuda()
        loss_dssim_pbr = torch.tensor(0.).cuda()
        loss_perceptual_pbr = torch.tensor(0.).cuda()
        loss_l1_pbr_deferred = torch.tensor(0.).cuda()
        loss_dssim_pbr_deferred = torch.tensor(0.).cuda()
        loss_perceptual_pbr_deferred = torch.tensor(0.).cuda()
        loss_rm_image_based = torch.tensor(0.).cuda()
        ...
        if self.iter > self.pbr_iteration:
            if self.occlusion_flag and (
                ...
                self.occlusion_flag = False
            # recon occlusion
            if self.GS_config.enable_occ_type == 'gaussian':
                # print("begin to use gaussian splatting to render occlusion")
                # deformed normal
                # normal_points = self.precompute_deformed_gaussian.get_normal
                normal_points = self.flip_normals(self.precompute_deformed_gaussian, c2w)
                points = self.precompute_deformed_gaussian.get_xyz
                ...
                # occlusion = torch.ones_like(roughness_map).permute(1, 2, 0)  # [H, W, 1]
                # irradiance = torch.zeros_like(roughness_map).permute(1, 2, 0)  # [H, W, 1]
                occlusion = torch.ones_like(self.precompute_deformed_gaussian.get_opacity)  # [N, 1]
                irradiance = torch.zeros_like(self.precompute_deformed_gaussian.get_opacity)  # [N, 1]

            # normal_mask = render_pkg["normal_mask"]  # [1, H, W]

            black_color = [0, 0, 0]
            black_background = torch.tensor(black_color, dtype=torch.float32, device="cuda")
            albedo_output = render_fast_2dgs(data, self.precompute_deformed_gaussian, self.GS_config.pipeline,
                                             black_background, scaling_modifier=1.0,
                                             override_color=self.precompute_deformed_gaussian.get_albedo)
            rough_metal_occ = torch.cat(
                [self.precompute_deformed_gaussian.get_roughness.reshape(-1, 1),
                 self.precompute_deformed_gaussian.get_metallic.reshape(-1, 1), occlusion.reshape(-1, 1)], dim=-1)
            rough_metal_occ_output = render_fast_2dgs(data, self.precompute_deformed_gaussian, self.GS_config.pipeline,
                                                      black_background, scaling_modifier=1.0,
                                                      override_color=rough_metal_occ)
            ...
            brdf_tv_loss = self.get_masked_tv_loss(
                gt_mask > 0.5,
                gt_image,  # [3, H, W]
                torch.cat([albedo_output['render'], rough_metal_occ_output['render'][:2]], dim=0),  # [5, H, W]
            )
            brdf_tv_weight = 1.0
            loss += brdf_tv_loss * brdf_tv_weight

            lambda_distill_image_rm = self.GS_C(self.iter, self.GS_config.opt.get('lambda_distill_image_rm', 0.))
            if data.roughness_img is not None and lambda_distill_image_rm > 0.:
                # distill data.normal_img
                # convert render_pkg["normal_map"] to normal_img
                # TODO check whether they are in the same space
                # import pdb;pdb.set_trace()
                pred_roughness = rough_metal_occ_output['render'][[0]].reshape(-1)
                pred_metallic = rough_metal_occ_output['render'][[1]].reshape(-1)
                pred_roughness = self.scene.distill_adapt_roughness_layer(pred_roughness.reshape(1, -1)).reshape(-1)
                pred_metallic = self.scene.distill_adapt_metallic_layer(pred_metallic.reshape(1, -1)).reshape(-1)
                valid_image_mask = data.original_mask.reshape(-1) > 0.5
                gt_roughness = data.roughness_img[0].reshape(-1)
                gt_metallic = data.metallic_img[0].reshape(-1)
                loss_rm_image_based +=  F.mse_loss(pred_roughness[valid_image_mask], gt_roughness[valid_image_mask])
                loss_rm_image_based +=  F.mse_loss(pred_metallic[valid_image_mask], gt_metallic[valid_image_mask])
                loss += lambda_distill_image_rm * (loss_rm_image_based)
            ...
            self.scene.cubemap.build_mips()  # build mip for environment light
            if self.rendering_type == 'forward_pbr':
                pbr_result_gs = pbr_shading_gs(deformed_gaussian=self.precompute_deformed_gaussian,
                                               camera_center=c2w[:3, 3],
                                               # TODO check if the camera center is correct c2w[:3, 3]
                                               light=self.scene.cubemap,
                                               occlusion=occlusion,  # [pc, 1]
                ...
                override_color = pbr_result_gs
                # TODO check shape should be [3, H, W]
                render_output = render_2dgs(data, self.precompute_deformed_gaussian, self.GS_config.pipeline, self.background, scaling_modifier = 1.0, override_color = override_color)
                pbr_result = render_output['render'].clamp(min=0.0, max=1.0)
                render_rgb = linear_to_srgb(pbr_result)
                # TODO check whether it is needed
                # render_rgb = torch.where(
                ...
                #     self.background[:, None, None],
                # )
                # TODO update weights (done)
                # pbr_render_loss = l1_loss(render_rgb, gt_image)
                # loss += pbr_render_loss
            elif self.rendering_type == 'deferred_pbr':
                # pass
                albedo_align_output = render_fast_2dgs(data, self.precompute_deformed_gaussian, self.GS_config.pipeline,
                                             self.background, scaling_modifier=1.0,
                                             override_color=self.precompute_deformed_gaussian.get_albedo)
                input_albedo = albedo_align_output["render"]
            ...
            loss_dssim_pbr = torch.tensor(0.).cuda()
            if lambda_l1 > 0.:
                loss_l1_pbr = l1_loss(render_rgb, gt_image)
            if lambda_dssim > 0.:
                loss_dssim_pbr = 1.0 - ssim(render_rgb, gt_image)
            loss += (lambda_l1 * loss_l1_pbr + lambda_dssim * loss_dssim_pbr)
            ...
            if lambda_perceptual > 0:
                ...
                y1, y2 = mask[1].min(), mask[1].max() + 1
                x1, x2 = mask[2].min(), mask[2].max() + 1
                fg_image = render_rgb[:, y1:y2, x1:x2]
                gt_fg_image = gt_image[:, y1:y2, x1:x2]

                loss_perceptual_pbr = self.loss_fn_vgg(fg_image, gt_fg_image, normalize=True).mean()
                loss += lambda_perceptual * loss_perceptual_pbr
            else:
                loss_perceptual_pbr = torch.tensor(0.).cuda()
            ...
            # loss_dssim_pbr_deferred = torch.tensor(0.).cuda()
            # if lambda_l1 > 0.:
            #     loss_l1_pbr_deferred = l1_loss(render_rgb, gt_image)
            # if lambda_dssim > 0.:
            #     loss_dssim_pbr_deferred = 1.0 - ssim(render_rgb, gt_image)
            # loss += (lambda_l1 * loss_l1_pbr_deferred + lambda_dssim * loss_dssim_pbr_deferred)
            #
            # # perceptual loss
            # lambda_perceptual = self.GS_C(self.iter, self.GS_config.opt.get('lambda_perceptual', 0.))
            # if lambda_perceptual > 0:
            ...
            #     gt_fg_image = gt_image[:, y1:y2, x1:x2]
            #
            #     loss_perceptual_pbr_deferred = self.loss_fn_vgg(fg_image, gt_fg_image, normalize=True).mean()
            #     loss += lambda_perceptual * loss_perceptual_pbr_deferred
            # else:
            #     loss_perceptual_pbr_deferred = torch.tensor(0.).cuda()
            #
            #
            ...
            if data.normal_img is not None and lambda_distill_image > 0.:
                ...
                # TODO check whether they are in the same space
                # import pdb;pdb.set_trace()
                pred_albedo = albedo_output['render'] # [3, H, W]
                if self.enable_albedo_adapt:
                    pred_albedo_adapt = pred_albedo.reshape(3, -1) * self.scene.albedo_adapt_params
                else:
                    pred_albedo_adapt = pred_albedo.reshape(3, -1)
                # import pdb; pdb.set_trace()
                # TODO check if gl_normal_img looks like data.normal_img
                # import torchvision
                # torchvision.utils.save_image(gl_normal_img, './gl_normal_img.png')
                ...
                # torchvision.utils.save_image(data.original_mask, './data_original_mask.png')
                # torchvision.utils.save_image(data.original_image, './data_original_image.png')
                loss_albedo_image_based = l1_loss(pred_albedo_adapt.reshape(3, -1)[:, valid_image_mask], data.albedo_img.reshape(3, -1)[:, valid_image_mask])
                # distill data.depth_value
                # # todo here the render_pkg["depth_map"] is unknown, we need to check whether data.depth_value is also unknown
                # loss_image_based += l1_loss(render_output["surf_depth"].reshape(-1)[valid_image_mask], data.depth_value.reshape(-1)[valid_image_mask])
                loss += loss_albedo_image_based * lambda_distill_image
            ...
            # brdf_tv_weight = 1.0
            # loss += brdf_tv_loss * brdf_tv_weight
            lamb_weight = 0.0025 # 0.005 # 0.001
            lamb_loss = (1.0 - self.precompute_deformed_gaussian.get_roughness).mean() + self.precompute_deformed_gaussian.get_metallic.mean()
        ...
        # regularization
        lambda_normal = self.GS_config.opt.lambda_normal if self.iter > 7000 else 0.0
        lambda_dist = self.GS_config.opt.lambda_dist if self.iter > 3000 else 0.0

        rend_dist = render_output["rend_dist"]
        rend_normal = render_output['rend_normal']
        surf_normal = render_output['surf_normal']
        # normal_error = (1 - (rend_normal * surf_normal).sum(dim=0))[None]
        # normal_loss = lambda_normal * (normal_error).mean()
        normal_loss = torch.zeros((1), device=loss.device, requires_grad=True)
        ...
        # regularization
        loss_reg = self.precompute_loss_reg
        ...
        if loss.isnan():
            ...
            log_loss = {
                ...
                'loss/loss_orientation': loss_orientation.item(),
                'loss/loss_rm_image_based': loss_rm_image_based.item(),
                # 'loss/min_scaling': min_scaling.item(),
                'loss/anisotropy_reg': anisotropy_reg.item(),
                'loss/scale_penalty': scale_penalty.item(),
                ...
                'loss/loss_albedo_image_based': loss_albedo_image_based.item(),
                'loss/pbr_render_loss': pbr_render_loss.item(),
                'loss/loss_l1_pbr': loss_l1_pbr.item(),
                'loss/loss_dssim_pbr': loss_dssim_pbr.item(),
                'loss/loss_perceptual_pbr': loss_perceptual_pbr.item(),
                'loss/loss_l1_pbr_deferred': loss_l1_pbr_deferred.item(),
                'loss/loss_dssim_pbr_deferred': loss_dssim_pbr_deferred.item(),
                'loss/loss_perceptual_pbr_deferred': loss_perceptual_pbr_deferred.item(),
            ...
            import pdb;pdb.set_trace()
        loss.backward()
        # if self.scene.converter.deformer.rigid.lbs_network.lin0.weight.grad.isnan().any():
        #     print("Nan: rigid network")
        #     # import pdb;pdb.set_trace()
        #
        # if self.scene.converter.deformer.rigid.lbs_network.lin0.weight.grad.isnan().any() or self.scene.gaussians._xyz.grad.isnan().any():
        if self.scene.gaussians._xyz.grad.isnan().any():
            print("Nan: rigid network")
            print("Nan: rigid network")
            nan_flag = True
            self.scene.gaussians.prune_nan()
            self.scene.converter.optimizer.zero_grad(set_to_none=True)
            self.scene.gaussians.optimizer.zero_grad()
            self.scene.light_optimizer.zero_grad(set_to_none=True)
    ...
    def optimize_GS_step(self, render_pkg, data, distill_data):
        ...
        lambda_aiap_cov = self.GS_C(self.iter, self.GS_config.opt.get('lambda_aiap_cov', 0.))
        if lambda_aiap_xyz > 0. or lambda_aiap_cov > 0.:
            loss_aiap_xyz, loss_aiap_cov = full_aiap_loss_2dgs(self.scene.gaussians, render_pkg["deformed_gaussian"])
        else:
            loss_aiap_xyz = torch.tensor(0.).cuda()
            loss_aiap_cov = torch.tensor(0.).cuda()
        loss += lambda_aiap_xyz * loss_aiap_xyz
