...
def get_canonical_rays(camera) -> torch.Tensor:
    ...
    # TODO here only support cx cy is at the center of the image
...
# def get_canonical_rays_from_intrinsic(camera) -> torch.Tensor:
#     # NOTE: some datasets do not share the same intrinsic (e.g. DTU)
#     # get reference camera
#     ref_camera = camera
#     # TODO: inject intrinsic
...
#     # cen_x = W / 2
#     # cen_y = H / 2
#     cen_x = ref_camera.K[0, 2]
#     cen_y = ref_camera.K[1, 2]
#     tan_fovx = math.tan(ref_camera.FoVx * 0.5)
#     tan_fovy = math.tan(ref_camera.FoVy * 0.5)
#     focal_x = W / (2.0 * tan_fovx)
#     focal_y = H / (2.0 * tan_fovy)
...
#     # NOTE: it is not normalized
#     return camera_dirs.cuda()
def get_world_camera_rays_from_intrinsic(camera) -> torch.Tensor:
    ref_camera = camera
    H, W = ref_camera.image_height, ref_camera.image_width
    K = ref_camera.K
    c2w = torch.inverse(ref_camera.w2c_opencv).cpu().numpy()
    # c2w = torch.inverse(ref_camera.world_view_transform).cpu().numpy()
    rays_o, rays_d = make_rays(K, c2w, H, W)
    return torch.from_numpy(rays_d).cuda()
