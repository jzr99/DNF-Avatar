...
class PSEvaluator(nn.Module):
    ...
    def forward(self, rgb, rgb_gt, valid_mask=None):
    ...
    def evaluate_normal(self, inputs, targets, valid_mask):
...
class RANAEvaluator(nn.Module):
    def __init__(self):
        super().__init__()
        from systems.criterions import PSNR as PSNR_IA
        from systems.criterions import NormalError
        from systems.criterions import SSIM as SSIM_IA
        from systems.criterions import LPIPS as LPIPS_IA
        self.cuda()
        self.eval()
        self.criterions = {
            'psnr': PSNR_IA(),
    ...
    def forward(self, inputs, targets, valid_mask):
        ...
        inputs = inputs.permute(1, 2, 0) # hwc
        ...
        return {
            "psnr": pbr_psnr,
            "ssim": torch.from_numpy(np.array(pbr_ssim)),
            "lpips": pbr_lpips,
        }
