...
from models_GS.network_utils import VanillaCondMLP
...
class ColorMLP(ColorPrecompute):
    ...
    def compose_input(self, gaussians, camera):
        ...
        if self.non_rigid_dim > 0:
            ...
            if not hasattr(gaussians, "non_rigid_feature"):
                non_rigid_feature = torch.zeros(gaussians.get_xyz.shape[0], self.non_rigid_dim, device=features.device, dtype=features.dtype)
                features = torch.cat([features, non_rigid_feature], dim=1)
                # TODO check here
                # print("Warning: non_rigid_feature not found in gaussians")
            else:
...
class ColorMLPREF(ColorPrecompute):
    def __init__(self, cfg, metadata):
        super().__init__(cfg, metadata)
        d_in = cfg.feature_dim
    ...
    def compose_input(self, gaussians, camera):
        ...
        if self.use_cov:
            ...
            features = torch.cat([features, cov], dim=1)
        if self.use_normal:
            if gaussians.gs_type == '3dgs':
                scale = gaussians._scaling
                rot = build_rotation(gaussians._rotation)
                normal = torch.gather(rot, dim=2, index=scale.argmin(1).reshape(-1, 1, 1).expand(-1, 3, 1)).squeeze(-1)
                features = torch.cat([features, normal], dim=1)
            elif gaussians.gs_type == '2dgs':
                normal = gaussians.get_normal # normal in observation space
                features = torch.cat([features, normal], dim=1)
            else:
                raise NotImplementedError
        if self.sh_degree > 0:
            ...
            ref = reflect(-dir_pp, gaussians.get_normal)
            # dir_pp_normalized = dir_pp / (dir_pp.norm(dim=1, keepdim=True) + 1e-12)
            dir_embed = self.sh_embed(ref)
            features = torch.cat([features, dir_embed], dim=1)
        if self.non_rigid_dim > 0:
            # assert hasattr(gaussians, "non_rigid_feature")
            if not hasattr(gaussians, "non_rigid_feature"):
