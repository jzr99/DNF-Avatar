...
from models_GS.network_utils import get_skinning_mlp
...
class SMPLKNN(RigidDeform):
    ...
    #     p_idx = knn_ret.idx.squeeze() # (N, P1, K)
    #     pts_W = self.skinning_weights[p_idx, :]
    #
    #     return pts_W, pts_W

    def query_weights(self, xyz):
        knn_ret = ops.knn_points(xyz.unsqueeze(0), self.smpl_verts.unsqueeze(0), K=self.K, return_nn=False)
        distance_batch = knn_ret.dists
        index_batch = knn_ret.idx # (1, P1, K)
        distance_batch = torch.clamp(distance_batch, max=4)
        weights_conf = torch.exp(-distance_batch) # (1, P1, K)
        # distance_batch = torch.sqrt(distance_batch)
        weights_conf = weights_conf / weights_conf.sum(-1, keepdim=True) # (1, P1, K)
        weights_conf = weights_conf[0] # (P1, K)
        index_batch = index_batch[0] # (P1, K)
        weights = self.skinning_weights[index_batch, :] # (P1, K, J)
        weights = torch.sum(weights * weights_conf.unsqueeze(-1), dim=-2).detach() # (P1, J)
    ...
    def forward(self, gaussians, iteration, camera):
        ...
        n_pts = xyz.shape[0]
        _, pts_W = self.query_weights(xyz)
        ...
        if deformed_gaussians.gs_type == '2dgs':
            deformed_gaussians._rotation = matrix_to_quaternion(rotation_bar)
        elif deformed_gaussians.gs_type == '3dgs':
            normal = gaussians.get_normal
            normal_rotate = torch.matmul(T_fwd[:, :3, :3], normal.reshape(-1, 3, 1))
            deformed_gaussians._normal = normal_rotate.squeeze()
        else:
            raise NotImplementedError
...
class SkinningField(RigidDeform):
    ...
    def query_weights(self, xyz):
        ...
        # find the nearest vertex
        knn_ret = ops.knn_points(xyz.unsqueeze(0), torch.from_numpy(self.smpl_verts).float().cuda().unsqueeze(0))
        p_idx = knn_ret.idx.squeeze()
        pts_W_smpl = torch.from_numpy(self.skinning_weights).float().cuda()[p_idx, :]

        return pts_W, pts_W_smpl
