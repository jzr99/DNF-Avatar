...
class IntrinsicAvatarModel(BaseModel):
    ...
    def isosurface_deformed(self, smpl_param):
        smpl_param.update(self.pose_correction(smpl_param["index"]))
        self.deformer.prepare(smpl_param)
        ...
        mesh = self.geometry.isosurface_deform(self.deformer, cond, geometry_fn)
        return mesh
    ...
    def pbr_light_forward(
        ...
        with torch.no_grad():
            ...
            tr_mask = secondary_tr[..., 0] > 0.0
            # print("disable_occlusion")
            # tr_mask = secondary_tr[..., 0] > -1.0
    ...
    def eval_points_canonical(self, points):
        ...
        ret = geometry_fn(points)
        sdf = ret[0]
        sdf_grad_cano = ret[1]
        feature = ret[2]
        normal_cano = F.normalize(sdf_grad_cano, p=2, dim=-1, eps=1e-6)
        t_dirs = torch.zeros_like(normal_cano, device=normal_cano.device)
        valid = torch.ones_like(sdf, device=sdf.device, dtype=torch.bool)
        # here we use normal cano to compute radiance feature not normal world
        rgbs, rgb_feature = self.radiance(points, feature, t_dirs, normal_cano)
        ...
        return {'normal_cano': normal_cano, 'sdf_cano': sdf, 'valid_cano': valid, 'materials_cano': materials}

    def eval_points(self, points):
        self.cond = self.pose_encoder(
            self.deformer.get_rot_mats(), self.deformer.get_joints()
        )
        deformed_points = self.deformer.rigid_deformer.transform_pnts_w2s(points)
        ...
        # t_dirs = self.deformer.rigid_deformer.transform_dirs_s2w(t_dirs)
        # alphas = self.get_alpha(sdf, dists)
        t_dirs = torch.zeros_like(normal_world, device=normal_world.device)
        # Get radiance
        ...
        return {'normal_world': normal_world, 'sdf': sdf, 'valid': valid, 'materials': materials}
        ...
        if self.training:
            ...
            if self.with_in_surface_loss:
                ...
                (
                    ...
                    self.deformer.rigid_deformer.vs_template,
                    self.deformer.rigid_deformer.body_model.faces_tensor,
                    self.deformer.rigid_deformer.faces_to_include,
