...
class BaseImplicitGeometry(BaseModel):
    ...
    def isosurface_deform_(self, vmin, vmax, deformer, cond, geometry_fn):
        def batch_func(x):
            x = torch.stack([
                scale_anything(x[...,0], (0, 1), (vmin[0], vmax[0])),
                scale_anything(x[...,1], (0, 1), (vmin[1], vmax[1])),
    ...
    @torch.no_grad()
    def isosurface_deform(self, deformer, cond, geometry_fn):
        if self.config.isosurface is None:
            raise NotImplementedError
        bbox_cpu = self.bbox.cpu()
        bbox_np = bbox_cpu.numpy()
        mesh_coarse = self.isosurface_deform_(bbox_np[0], bbox_np[1], deformer, cond, geometry_fn)
        # import pdb;pdb.set_trace()
        vmin, vmax = mesh_coarse['v_pos'].amin(dim=0), mesh_coarse['v_pos'].amax(dim=0)
        vmin_ = (vmin - (vmax - vmin) * 0.1).clamp(bbox_cpu[0], bbox_cpu[1])
        vmax_ = (vmax + (vmax - vmin) * 0.1).clamp(bbox_cpu[0], bbox_cpu[1])
        mesh_fine = self.isosurface_deform_(vmin_, vmax_, deformer, cond, geometry_fn)
        return mesh_fine
