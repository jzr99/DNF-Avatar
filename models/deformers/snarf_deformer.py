...
def get_predefined_rest_pose(cano_pose, device="cuda"):
    ...
    if cano_pose.lower() == "da_pose":
        ...
        body_pose_t[:, 47] = -0.8
        body_pose_t[:, 50] = 0.8
    elif cano_pose.lower() == "star_pose":
        body_pose_t[:, 2] = torch.pi / 4
        body_pose_t[:, 5] = -torch.pi / 4
    elif cano_pose.lower() == "ps_pose":
        body_pose_t[:, 2] = 0.2
        body_pose_t[:, 5] = -0.2
    elif cano_pose.lower() == "ps_female_3_pose":
        body_pose_t[:, 2] = 0.2
        body_pose_t[:, 5] = -0.2
        body_pose_t[:, 47] = -0.2
        body_pose_t[:, 50] = 0.2
...
class SNARFDeformer():
    ...
    def transform_pnts_w2s(self, pnts):
        """transform pnts from world to smpl coordinate system"""
        w2s = self.w2s.detach()
        deformed_pnts = (pnts[:, :3] @ w2s[0, :3, :3].permute(1, 0)) + w2s[0, None, :3, 3]
