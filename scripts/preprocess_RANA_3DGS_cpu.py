...
from scripts.smplx import SMPL
# body models
...
if __name__ == "__main__":
    ...
    body_model_smpl = SMPL(model_path="./data/SMPLX/smpl", gender=gender)
    ...
    for img_idx, img_file in enumerate(img_files):
        ...
        poses_torch = torch.from_numpy(smpl_pose)
        betas_torch = torch.from_numpy(smpl_betas)
        global_orient_torch = torch.from_numpy(smpl_global_orient)
        ...
        transl_torch = torch.from_numpy(smpl_transl.reshape(1, -1))
        ...
        if args.split == "test":
            ...
            global_orient_torch = torch.from_numpy(smpl_global_orient).float()
        ...
        body_model = BodyModel(bm_path='body_models/smpl/male/model.pkl', num_betas=10, batch_size=1)
        body = body_model(root_orient=global_orient_torch, pose_body=poses_torch[:, :63],
                          betas=betas_torch, trans=transl_torch)
        import pdb; pdb.set_trace()
