...
if __name__ == "__main__":
    ...
    verts_dir = os.path.join(data_dir, "vertices")
    cameras = annots["cams"]
    ...
    for cam_idx, cam_name in enumerate(cam_names):
        ...
        albedo_out_dir = os.path.join(out_dir, "albedos/{}".format(cam_name))
        os.makedirs(albedo_out_dir, exist_ok=True)
        normal_out_dir = os.path.join(out_dir, "normals/{}".format(cam_name))
        ...
        for img_idx, img_file in enumerate(img_files):
            ...
            if cam_idx == 0:
                ...
                global_orients.append(root_orient)
                body_poses.append(poses[:, 3:])
            ...
            # copy and rename vertex files
            shutil.copyfile(
                os.path.join(verts_dir, "{}.npy".format(idx)),
            ...
            albedo_file = os.path.join(
                "/media/sfwang/data/Datasets/synthetic_human_processed",
                args.seqname,
    ...
    params = {
        ...
        "pose": np.concatenate(
            [
                np.concatenate(global_orients, axis=0),
                np.concatenate(body_poses, axis=0),
            ],
            axis=-1,
        ),
        "transl": np.concatenate(transls, axis=0),
    }
    np.save(
        out_filename,
        params,
