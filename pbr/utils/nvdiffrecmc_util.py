...
def dilate(x, x_avg, mask, N):
    def _gaussian():
        ...
        grid_y, grid_x = torch.meshgrid(torch.linspace(-1, 1, N, dtype=torch.float32, device="cuda"),
                                        torch.linspace(-1, 1, N, dtype=torch.float32, device="cuda"))
    ...
    def _w(c, cN):
        return torch.stack(
            list(_gaussian() if i == c else torch.zeros(N, N, dtype=torch.float32, device="cuda") for i in range(cN)),
            dim=0)

    epsilon = 1e-6
    weights = torch.stack(list(_w(i, x.shape[3]) for i in range(x.shape[3])), dim=0)
    mask_flt = torch.nn.functional.conv2d(mask.permute(0, 3, 1, 2), weights[0:1, 0:1, ...], padding=N // 2).permute(0,
                                                                                                                    2,
                                                                                                                    3,
                                                                                                                    1)
...
def load_image(fn) -> np.ndarray:
    ...
    elif fn.endswith('hdr'):
        img = read_hdr(fn)
    else:
        raise NotImplementedError("wrong image type")
